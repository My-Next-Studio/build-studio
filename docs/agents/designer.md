...
4. **Save design artifacts and update the PRD (MANDATORY — do this before ending your session):**
   - Save the `.pen` file to disk at the path specified in the PRD's §10 table (e.g., `design/prd-032-payment-checkout.pen`). If no path is specified, use the pattern `design/prd-NNN-short-name.pen`.
   - Verify the file was written: read it back and confirm it exists and is non-empty.
   - Export PNGs saved to `docs/design/` for reference
   - Both desktop (1440px) and mobile (390px) if the feature has responsive layouts
   - **Update the PRD's §10 companion spec table** — set the visual design row to status "Done" with the correct file path. This is how the execution workflow validates that a design exists before planning tasks.
...
- **Always save the .pen file before ending your session** — this is the most important deliverable. If the file isn't on disk, the execution workflow will block at the planning step. Verify the file exists after saving.
